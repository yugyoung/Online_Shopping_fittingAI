...
D_BC = nn.DataParallel(Discriminator(input_shape = (TRAIN_BATCH_SIZE, 3,256,256))).cuda()
...
if __name__ == '__main__':
    ...
    for epoch in range(1, EPOCH +1):
        for (batch_idx, target) in enumerate(train_loader): 
            ...
            save_out = "results/train5_10/outputs"
            ...
            #------------------cycleGan stage two
            cycle_fCB_cloth = torch.cat([cycle_fake_CB, fake_CB], dim=1)
            cycle_fBC_cloth = torch.cat([fake_CB, cycle_fake_BC], dim=1)
            cycle_tcloth_recv = G_BC.forward(cycle_fCB_cloth).cuda()
            cycle_tcloth_recv_2 = G_BC.forward(cycle_fBC_cloth).cuda()
            ...
            loss_identity_BC = criterion_identity(cycle_tcloth_recv, fake_CB) #L1 loss
            loss_identity_BC += criterion_identity(cycle_tcloth_recv_2, fake_CB) #L1 loss
            loss_identity_BC /= 2 #loss_identity = (loss_id_A + loss_id_B) / 2
            
            loss_identity = (loss_identity_CB + loss_identity_BC)
            ...
            valid = torch.tensor(np.ones((fake_CB.size(0), *D_BC.module.output_shape)), requires_grad=False, dtype=torch.float32).cuda()
            loss_GAN_BC = criterion_GAN(D_BC(cycle_tcloth_recv), valid)
            loss_GAN_BC += criterion_GAN(D_BC(cycle_tcloth_recv_2), valid)
            loss_GAN_BC /= 2 #loss_GAN = (loss_GAN_AB + loss_GAN_BA) / 2

            loss_GAN = (loss_GAN_CB + loss_GAN_BC)
            ...
            fBC_body = torch.cat([fake_BC, body], dim=1) #옷: body, 몸: body
            fCB_body = torch.cat([body, fake_CB], dim=1) #옷: body, 몸: body
            ...
            pred_real_cloth = D_BC(fake_CB)
            pred_fake_cloth = D_BC(cycle_tcloth_recv)
            pred_fake_cloth_2 = D_BC(cycle_tcloth_recv_2)
            
            loss_D_BC_real = criterion_GAN(pred_real_cloth, torch.ones_like(pred_real_cloth))
            loss_D_BC_fake = criterion_GAN(pred_fake_cloth, torch.zeros_like(pred_fake_cloth))
            loss_D_BC_fake_2 = criterion_GAN(pred_fake_cloth, torch.zeros_like(pred_fake_cloth))
            loss_D_BC = 0.5 * (loss_D_BC_real + loss_D_BC_fake) + 0.5 * (loss_D_BC_real + loss_D_BC_fake_2)
