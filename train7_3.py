...
D_fCB = nn.DataParallel(Discriminator(input_shape = (TRAIN_BATCH_SIZE, 3,256,256))).cuda() #warping
...
optimizer_D_fCB = torch.optim.Adam(D_fCB.parameters(), lr=LR)
...
lr_scheduler_D_fCB = torch.optim.lr_scheduler.LambdaLR(
    optimizer_D_fCB, lr_lambda=LambdaLR(EPOCH, 0, EPOCH*0.5).step
)
...
if __name__ == '__main__':
    ...
    for epoch in range(1, EPOCH +1):
        for (batch_idx, target) in enumerate(train_loader): 
            ...
            save_out = "results/train7_3_10/outputs"
            ...
            #------------------loss
            #D_fCB loss
            optimizer_D_fCB.zero_grad()
            fCB_body = torch.cat([fake_CB, body], dim=1).cuda()
            _fake_CB = G_CB.forward(fCB_body).cuda()

            #identity loss
            loss_identity_D_fCB = criterion_identity(fake_CB, _fake_CB) #L1 loss
            loss_identity_D_fCB.backward(retain_graph=True)
            optimizer_D_fCB.step()
            ...
            loss_D_CB_real = criterion_GAN(pred_real_cloth, torch.ones_like(pred_real_cloth)) #BCE
