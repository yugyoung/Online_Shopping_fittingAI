...
if __name__ == '__main__':
    ...
    for epoch in range(1, EPOCH +1):
        for (batch_idx, target) in enumerate(train_loader): 
            ...
            save_out = "results/train7_2_10/outputs"
            ...
            #print('target_body.shape: ',target_body.shape)ㅉ
            ...
            loss_identity_CB_body = criterion_identity(tbody_recv, body) #L1 loss
            loss_identity_CB_body += criterion_identity(tbody_recv_2, body) #L1 loss
            loss_identity_CB_body /= 2 #loss_identity = (loss_id_A + loss_id_B) / 2
            
            loss_identity = (loss_identity_CB + loss_identity_CB_body)
            ...
            valid = torch.tensor(np.ones((body.size(0), *D_CB.module.output_shape)), requires_grad=False, dtype=torch.float32).cuda()
            loss_GAN_CB_body = criterion_GAN(D_CB(tbody_recv), valid)
            loss_GAN_CB_body += criterion_GAN(D_CB(tbody_recv_2), valid)
            loss_GAN_CB_body /= 2 #loss_GAN = (loss_GAN_AB + loss_GAN_BA) / 2

            loss_GAN = (loss_GAN_CB + loss_identity_CB_body)
            ...
            save_inp_oup = torch.cat([_G_CB_output, _cloth_body], dim=0).cuda()
            ...
            pred_real_body = D_CB(body)
            pred_fake_body = D_CB(tbody_recv)
            pred_fake_body_2 = D_CB(tbody_recv_2)
            ...
            loss_D_CB_fake_2 = criterion_GAN(pred_fake_body, torch.zeros_like(pred_fake_body))
            loss_D_CB += 0.5 * (loss_D_CB_real + loss_D_CB_fake) + 0.5 * (loss_D_CB_real + loss_D_CB_fake_2)
