...
class BottleNeck(nn.Module): #ResNet-50부터
    expansion = 4
    def __init__(self, in_channels, out_channels, stride=1):
        super().__init__()
...
class PatchGAN_Discriminator(nn.Module):
    def __init__(self, channels=3):
        ...
        self.Conv = nn.Sequential(
            ...
            spectral_norm(nn.Conv2d(256, 1, kernel_size=4, stride=2, padding=1, bias=True)),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
        )
...
#ref: https://github.com/andrewjong/ShineOn-Virtual-Tryon/blob/master/models/networks/cpvton.py      
class Encoder(nn.Module):
    def __init__(self, channels=3):
        super(Encoder, self).__init__()
        #bin = functools.partial(Normlayer, affine=True)
        self.Encoder_Conv = nn.Sequential(
            nn.Conv2d(channels, 512, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True),
...
class Decoder(nn.Module):
    def __init__(self, in_channels=128, filters=128, kernel_size=3, stride=1, padding=1):
        ...
        self.Decoder_conv = nn.Sequential(
            ...
            nn.Upsample(scale_factor=5, mode='bicubic'),
            nn.ReLU(),
            nn.Conv2d(2*filters, 4*filters, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(4*filters,1, kernel_size=3, stride=3, padding = 1)
        )
    def forward(self,inputs):
        output = self.Decoder_Conv(inputs) # batch_size x 512 x 10 x 6
        return output
