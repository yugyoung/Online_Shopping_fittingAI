...
def content_perceptual(perceptual, perceptual_converted):
        return  nn.MSELoss(perceptual[-1], perceptual_converted[-1])
def style_perceptual(style_gram, style_gram_converted):
        style_percptual_loss = 0
        for gr in range(len(style_gram)):
            style_percptual_loss += 5e4 * criterion_vgg(style_gram[gr], style_gram_converted[gr])
