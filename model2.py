...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True): #bilinear interpolation
...
#           RESNET
##############################
class CocosEnc(nn.Module):
    def __init__(self, in_features=256):
        super(CocosEnc, self).__init__()
...
class GeneratorResNet(nn.Module):
    def __init__(self, input_shape, num_residual_blocks=6):
        ...
        #channels = input_shape[1] #RGB = 3

        # Initial convolution block
        #out_features = 64
        self.conv1 = nn.Sequential(
            nn.ReflectionPad2d(3),
            nn.Conv2d(6, 64, 7), #원래 6(n_chanels),64,7
        ...
        self.last_layer = nn.Sequential(
            nn.ReflectionPad2d(3), 
            nn.Conv2d(64, 3, kernel_size=7, stride=1, padding=0), #[4, 3, 56, 56]
            nn.Tanh(),
        )

        #self.upsample = nn.ConvTranspose2d(3, 3, 7, stride=4, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.last_layer(x)
        #print("6 :",x.shape)
        #output = self.upsample(x, output_size=(4,3,256,256))
        return x
...
class Discriminator(nn.Module):
    def __init__(self, input_shape):
        ...
        batchs, channels, height, width = input_shape
...
class BasicBlock(nn.Module): #ResNet-18,34
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        # BatchNorm�� bias�� ���ԵǾ� �����Ƿ�, conv2d�� bias=False�� �����մϴ�.
...
class BottleNeck(nn.Module): #ResNet-50����
    expansion = 4
    def __init__(self, in_channels, out_channels, stride=1):
        super().__init__()
