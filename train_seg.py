...
vgg = nn.DataParallel(VGG19()).cuda()
...
if __name__ == '__main__':
    ...
    for epoch in range(1, EPOCH +1):
        for (batch_idx, target) in enumerate(train_loader): 
            ...
            save_out = "results/train_seg_ad_Cloth_10_2/"
            ...
            #vgg loss
            vgg_feature_img1 = vgg(body)
            vgg_feature_img2 = vgg(tbody)
            vgg_feature_fake1 = vgg(fake_body)
            gram_img1 = [gram(fmap) for fmap in vgg_feature_img1[:-1]]
            gram_img2 = [gram(fmap) for fmap in vgg_feature_img2[:-1]]
            gram_fake1 = [gram(fmap) for fmap in vgg_feature_fake1[:-1]]
            content_loss = criterion_vgg(vgg_feature_fake1[-1], vgg_feature_img1[-1]) #tbody, G1
            style_loss = style_perceptual(gram_fake1, gram_img2) #G1,body / body의 옷 스타일을 fake1에다가 주고 싶어.
            # Total loss
            loss_G = loss_GAN + LAMBDA_ID * loss_identity + style_loss
    ...
    #torch.save(G_seg.state_dict(),"models/G_seg_100.pt")
    #torch.save(D_seg.state_dict(),"models/D_seg_100.pt")
